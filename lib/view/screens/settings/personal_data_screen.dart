                  ...
                  return SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // FIX: previously always fell back to a text/letter placeholder
                        // (_SignaturePreview/_StampPreview) whenever the person hadn't picked
                        // their own yet — now falls back to Dyooni's REAL bundled default
                        // signature/stamp images instead, so the preview (and the generated PDF,
                        // see pdf_report_service.dart) always shows a proper-looking signature
                        // and stamp, not placeholder text. The old widgets are kept as the
                        // graceful fallback for the rare case the bundled asset itself is missing
                        // (same defensive pattern as AppLogo's own errorBuilder).
                        _MediaToggleRow(
                          ...
                          preview: _signaturePath != null
                              ...
                              : Image.asset(
                                  'assets/icons/default_signature.png',
                                  width: 60,
                                  height: 36,
                                  fit: BoxFit.contain,
                                  errorBuilder: (context, error, stackTrace) => const _SignaturePreview(),
                                ),
                        ),
                        const SizedBox(height: 12),
                        _MediaToggleRow(
                          ...
                          preview: _stampPath != null
                              ...
                              : ClipRRect(
                                  borderRadius: BorderRadius.circular(22),
                                  child: Image.asset(
                                    'assets/icons/default_stamp.png',
                                    width: 44,
                                    height: 44,
                                    fit: BoxFit.cover,
                                    errorBuilder: (context, error, stackTrace) => const _StampPreview(),
                                  ),
                                ),
                        ),
                        const SizedBox(height: 24),
...
/// Now only a SAFETY-NET fallback (see the doc comment above where this is used): the real,
/// bundled default signature image is what normally shows until the person draws their own.
...
/// Dyooni's own placeholder stamp — a simple drawn badge. Now only a SAFETY-NET fallback (see the
/// doc comment above where this is used): the real, bundled default stamp image is what normally
/// shows until the person picks their own.
