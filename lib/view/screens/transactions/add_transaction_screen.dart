...
class _AddTransactionScreenState extends ConsumerState<AddTransactionScreen> {
  ...
  AccountDirection _direction = AccountDirection.debit; // may be overridden by general settings, see initState
  ...
  void initState() {
    ...
    if (existing != null) {
      ...
      // الاتجاه الافتراضي عند إضافة عملية جديدة يأتي من الإعدادات العامة — انظر
      // logic/settings/general_settings_provider.dart.
  ...
  AccountDirection _resolveDefaultDirection(GeneralSettings settings) => switch (settings.defaultDirection) {
        DefaultDirectionOption.credit => AccountDirection.credit,
        DefaultDirectionOption.debit => AccountDirection.debit,
        DefaultDirectionOption.keepLast =>
          settings.lastUsedDirectionIsCredit == true ? AccountDirection.credit : AccountDirection.debit,
      };
  ...
  void _resetForm() {
    ...
    setState(() {
      _date = DateTime.now();
      final settings = ref.read(generalSettingsProvider).value;
      _direction = settings != null ? _resolveDefaultDirection(settings) : AccountDirection.debit;
      _amountError = null;
    });
  }
  ...
  Future<void> _save({required bool keepAdding}) async {
    ...
    try {
      ...
      unawaited(
        ref.read(generalSettingsProvider.notifier).recordLastUsedDirection(isCredit: _direction == AccountDirection.credit),
      );
      if (!mounted) return;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              if (showAmountInWords) AmountInWords(amountText: _amountController.text),
              const SizedBox(height: 6),
              Row(
                children: [
                  Text(
