...
class PdfReportService {
  ...
  /// Shared by the signature AND stamp: try the person's own custom file first, and only if
  /// that's absent (or unreadable) fall back to Dyooni's own bundled default image at
  /// [defaultAssetPath]. If EVEN the bundled default can't be loaded (asset not actually present
  /// yet on disk), returns `null` — the report simply omits that element rather than crashing
  /// generation, the same "never break the whole report over one bad file" principle already
  /// used by [_loadLogo]/[_loadAttachments].
  Future<pw.MemoryImage?> _loadCustomOrDefaultImage(String? customPath, String defaultAssetPath) async {
    try {
      if (customPath != null) {
        return pw.MemoryImage(await File(customPath).readAsBytes());
      }
    } catch (_) {
      // Falls through to the bundled default below.
    }
    try {
      final data = await rootBundle.load(defaultAssetPath);
      return pw.MemoryImage(data.buffer.asUint8List());
    } catch (_) {
      return null;
    }
  ...
  pw.Widget? _buildSignatureStampSection({
    ...
    if (signatureImage == null && stampImage == null) return null;

    pw.Widget box(String label, pw.MemoryImage? image, double width, double height) {
      if (image == null) return pw.SizedBox(width: width);
      return pw.Column(
    ...
    // Stamp is roughly square (badge-shaped); signature is wider/shorter (a written line) — the
    // two boxes are deliberately sized differently to match what each kind of image actually
    // looks like, rather than forcing both into one identical square.
    return pw.Padding(
      ...
      child: pw.Row(
        ...
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          box(stampLabel, stampImage, 120, 120),
          box(signatureLabel, signatureImage, 170, 100),
        ],
      ),
    );
  }
  ...
  /// a highlighted totals row, matching the reference report's exact column set. Ends with the
  /// stamp/signature section (see [_buildSignatureStampSection]) when enabled in [personalData].
  Future<Uint8List> buildSummaryTotals({
    ...
    final signatureImage = personalData.signatureEnabled
        ? await _loadCustomOrDefaultImage(personalData.signaturePath, 'assets/icons/default_signature.png')
        : null;
    final stampImage = personalData.stampEnabled
        ? await _loadCustomOrDefaultImage(personalData.stampPath, 'assets/icons/default_stamp.png')
        : null;
    final signatureStampSection = _buildSignatureStampSection(
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          if (signatureStampSection != null) signatureStampSection,
        ],
      ),
    );
  ...
  /// Ends with the stamp/signature section (see [_buildSignatureStampSection]) when enabled in
  /// [personalData].
