...
void main() {
  ...
  testWidgets('shows all three export actions with their real labels', (tester) async {
    when(() => accountsRepo.getAccounts()).thenAnswer((_) async => []);
    when(() => txRepo.getTransactions()).thenAnswer((_) async => []);
    ...
    // on-device testing is still needed for the export buttons themselves.
    expect(find.text(l10n.reportsExportPdf), findsOneWidget);
    expect(find.text(l10n.reportsExportExcel), findsOneWidget);
    expect(find.text(l10n.reportsShareWhatsapp), findsOneWidget);
  });
}
